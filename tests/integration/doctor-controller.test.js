...
const mockToken = jwt.sign({ id: mockDoctorId, role: "doctor" }, process.env.JWT_SECRET, {
...
describe("Doctor controller", () => {
  ...
  test("Get appointment", async () => {
    ...
    const res = await request(app).get(`/api/doctor/${mockAppointmentId}`).set("Authorization", `Bearer ${mockToken}`);
  ...
  test("Get all appointments", async () => {
    ...
    const res = await request(app).get(`/api/doctor/`).set("Authorization", `Bearer ${mockToken}`);
